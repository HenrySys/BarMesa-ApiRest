...
export class AddOrdersComponent implements OnInit {
  ...
  submitOrder() {
    ...
    if (pedidoStor) {
      let pedido: any = JSON.parse(pedidoStor);
      let idPedido = +pedido.id; // Obtener el id del pedido desde el localStorage
      ...
      if (pedidosProductos.length > 0) {
        ...
        pedidosProductos.forEach((producto: any) => {
          // Crear un nuevo objeto pedidoProducto
          pedidoProducto = (
            producto.idPedido,    // idPedido
            idPedido,             // idPedido (tomado de la variable)
            producto.idProducto,  // idProducto
            '',                    // Descripción vacía (se puede agregar si tienes la descripción del producto)
            producto.estado        // Estado del producto
          );

          // Llama al servicio para guardar el pedidoProducto en la base de datos
          this._PedidoProductoService.store(pedidoProducto).subscribe({
            next: (response: any) => {
      ...
      // Si no hay un pedido en el localStorage, mostrar un mensaje de error
      Swal.fire({
        icon: 'error',
        title: 'No hay pedido para guardar',
        showConfirmButton: false,
        timer: 1500
      });
    }
